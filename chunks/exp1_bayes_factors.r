...
if (FALSE) # this code is meant to be run manually
{
  ...
  load_model <- function(prior, region, dv) {
    fname <- sprintf('../workspace/models_eye_bf/%s/%s_%s.rds', prior, region, dv )
    print(fname)
    readRDS(fname)
  }
  ...
  for (region in c("np2", "spillover")) {
    for (dv in c("FPRT", "RPD", "TFT", "regression")) {
      ...
      for (prior in c("prior_1", "prior_0.5", "prior_0.25", "prior_0.1"))
      {
...
df_bf_eye <-
  bf_eye %>% plyr::ldply(function(lst) {
    res <- lst %>% plyr::ldply(function(lst) {
      res <- lst %>% plyr::ldply(function(lst) { lst$bf }, .id = "measure")
    }, .id = "region")
