...
%   - 36 ms (TFT), spill-over, Experiment 2 (RCs) | percentage: 0.063 = 36*3/(542+578+601) -> log(1.063) = 0.061
...
Importantly, Bayes Factors depend on the prior distribution of the parameter of interest, and thus require a specific hypothesis about the size of the effect in order to quantify the evidence for its absence. As table \ref{tab:prior_widths} shows, the prior we used for the estimation of slope coefficients ($N(0, 0.2)$) is rather wide, as it assigns a $95\%$ prior probability to an effect size between $-33\%$ and $+49\%$ of the overall intercept, and a $68\%$ prior probability to an effect size between $-18\%$ and $+22\%$ of the overall intercept.\footnote{The specification of an effect size as a percentage of the intercept is a consequence of the fact that additive effects on the log-scale are multiplicative in their original units.} That is, when the average reading time is $400\,ms$, the prior stipulates that the effect is most likely to be between $-132\,ms$ and $196\,ms$, and more likely than not between $-72\,ms$ and $88\,ms$.

In order to quantify the evidence against the hypothesis of a reading time difference between ambiguous and N1 attachment conditions, we fitted all reading-time models for a range of different priors \textit{for the difference between the preferred N1 attachment condition and the ambiguous condition} ($\sigma=0.2$, $\sigma=0.1$, $\sigma=0.05$, $\sigma=0.02$), which correspond to different assumptions about hypothesized effect sizes (cf. Table \ref{tab:prior_widths}). We then used the Savage-Dickey method implemented in \emph{brms} \citep{R-brms_a} in order to compute Bayes Factors.

\begin{table}[]
    \centering
    \begin{tabular}{c||c|c}
        Prior Width & {Central $68\%$ of Probability Mass} & {Central $95\%$ of Probability Mass} \\
        \hline
         $\sigma = 0.2$ & $[-18\%; +22\%]$ & $[-33\%; +49\%]$ \\
         $\sigma = 0.1$ & $\pm 10\%$ & $[-18\%; +22\%]$ \\
         $\sigma = 0.05$ & $\pm 5\%$ & $\pm 10\%$ \\
         $\sigma = 0.02$ & $\pm 2\%$ & $\pm 4\%$
    \end{tabular}
    \caption{Allocation of probability mass over effect sizes for different prior widths.}
...
\caption{Estimated marginal means and 95\% credible intervals of first-pass regression probabilities and back-transformed log-reading time measures FPRT, RPD, and TFT for the critical regions by condition. All estimates and credible intervals were obtained using the \emph{emmeans} package 
...
The average comprehension questions accuracy was \(78.8\%\). Figure \ref{fig:EyeRTsPlot} show a summary of the estimated marginal means and 95\% credible intervals of the eye-tracking measures computed for the the critical regions:
...
Figure \ref{fig:EyeBFs1} shows the Bayes Factors in favor of the hypothesis of no difference between NP1 attachment conditions and ambiguous conditions. While the Bayes Factor indicates a small amount of evidence for hypothesis of no difference between ambiguous and N1 attachment conditions for all priors, it fails to provide unambiguous evidence in favor of that hypothesis.
...
%
%We excluded seven participants' data from further analysis because they failed to answer more than $25\%$ of the comprehension questions correctly.
...
%All were participants in Experiment 2B. 
We also excluded all trials in which the reading time on one of the words between the pre-critical region and the spill-over region was shorter than $150\,ms$ or longer than $6000\,ms$, resulting in exclusion of further $1.2\%$ of the data ($95$ out of $7,644$ trials).
...
(i) the availability of NP1 and N2 for RC attachment, (ii) the presence of a relative clause (\emph{relative clause} vs.~\emph{control}), as well as (iii) the interactions between the presence of a relative clause and the availability of NP1 and NP2 for RC attachment.
...
%By-item random slopes of word length were not included in the models for critical regions N1 and N2, because they would be collinear with the effect of the grammatical number of those nouns.
...
As in Experiment 1, we used the Savage-Dickey method to calculate Bayes Factors quantifying the evidence against the presence of an ambiguity advantage for a range of priors. To this end, we fitted all models with priors with $\sigma=0.2$, $\sigma=0.1$, $\sigma=0.05$, $\sigma=0.02$ for the difference between the preferred N1 attachment condition and the ambiguous condition.
...
\begin{figure}
\includegraphics[width=\maxwidth]{./figure/sprEMM} 
\caption{Estimated marginal means and 95\% credible intervals of first-pass regression probabilities and back-transformed log-reading time measures FPRT, RPD, and TFT for the critical regions by condition. All estimates and credible intervals were obtained using the \emph{emmeans} package 
\protect{\citep{emmeans}}.
...
The analysis shows a moderate amount of evidence against a model with a $N(0, 0.2)$ prior for the difference between ambiguous and N1 attachment, as well as against a model with a $N(0, 0.1)$ prior. While the evidence against models with the priors $N(0, 0.05)$ and $N(0, 0.02)$ is weaker, it points in the same direction.
...
($\Sexpr{ sprintf("%0.3f", mean(stats_spr$samples_spr_np2[["delta_n1attachment_m_amb_ms"]] > min(prior_amb_adv_ms))) }$), and 
% to-do: XXX from here!
preferred it was noteworthy at the spill-over region ($\Sexpr{ mean(stats_spr$samples_spr_spillover[["delta_n1attachment_m_amb_ms"]] > min(prior_amb_adv_ms)) }$).


%Importantly, our data did not indicate the presence of an ambiguity advantage. We had hypothesized that such an effect would occur either at N2 or at the spillover position. The posterior probability of an ambiguity advantage of the expected magnitude at N2 was very low
($\Sexpr{ sprintf("%0.3f", mean(stats_spr$samples_spr_np2[["delta_n1attachment_m_amb_ms"]] > min(prior_amb_adv_ms))) }$), and while it was noteworthy at the spill-over region
($\Sexpr{ mean(stats_spr$samples_spr_spillover[["delta_n1attachment_m_amb_ms"]] > min(prior_amb_adv_ms)) }$), its magnitude at the spill-over region would need to compensate for the NP1 attachment speedup in order to yield a net ambiguity advantage.
...
Here, we proposed two different interpretations of the Swets et al. (2008) strategic underspecification account and discussed how the ambiguity advantage follows from their specific sets of assumptions.  
